...
export const Register = (handleName) => {
    ...
    let registerUser = async (e) => {
        e.preventDefault();
        let user = await signUp(email, pwd);
        // Then update the user's profile to add the name
    ...
    return (
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            <DialogTrigger className="px-8 py-3 text-sm rounded-lg font-bold text-primary-200 border-dashed border-2 border-primary-200 f transition-all ease-in-out duration-300 transform hover:-translate-y-2">
                SIGN UP
            </DialogTrigger>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        <form className="flex flex-col gap-4" method="POST" onSubmit={registerUser}>
